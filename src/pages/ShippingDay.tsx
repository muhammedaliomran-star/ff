...
function DayCard({ day }: { day: DaySummary; key?: React.Key }) { const isToday = day.key === todayKey; return <div className={`relative overflow-hidden rounded-2xl border bg-card/70 p-4 transition-[border-color,background-color] ${isToday ? "border-foreground/20 bg-foreground/[0.04]" : "border-foreground/10"}`}><div className="flex items-center justify-between gap-2"><span className="text-sm font-bold">{dayLabel(day.key)}</span>{isToday && <span className="rounded-full bg-foreground/[0.06] px-2 py-1 text-xs font-black text-foreground">النهارده</span>}</div><p className="mt-4 text-3xl font-black tabular-nums">{money(day.total)} <span className="text-xs font-bold text-muted-foreground">ج.م</span></p><div className="mt-4 grid grid-cols-2 gap-2 border-t border-border/60 pt-3 text-xs"><div><p className="text-success">مدفوع</p><p className="mt-1 font-black tabular-nums">{money(day.paid)}</p></div><div><p className="text-warning">معلّق</p><p className="mt-1 font-black tabular-nums">{money(day.due)}</p></div></div><div className="mt-3 flex items-center justify-between text-xs text-muted-foreground"><span>{day.count} شحنة</span><span>{day.active ? `${day.active} نشطة` : "مكتملة"}</span></div>{day.invoiceIds[0] && <Link to={`/shipping?invoice=${day.invoiceIds[0]}`} className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-foreground hover:underline">عرض الشحنات <ChevronLeft className="h-3 w-3" /></Link>}</div>; }
